<?php
/**
 * movimientos_crear.php
 * ACTUALIZACIÓN M:N:
 *   - Encargado puede elegir cualquiera de SUS bodegas como origen.
 *   - Destino: cualquier bodega activa.
 */
...
if (is_encargado()) {
    $misBodegas = user_bodegas();
    foreach ($misBodegas as $b) $misBodegasIds[] = (int)$b['id'];
...
$productos = $pdo->query("
    SELECT DISTINCT p.id, p.codigo, p.nombre,
           um.nombre AS unidad_nombre, tp.nombre AS tipo_nombre
    FROM   productos p
...
// Stock: para encargado, solo sus bodegas (si tiene muchas, igual todas)
if (is_encargado() && $misBodegasIds) {
    ...
    $stmtStock = $pdo->prepare("
        ...
        FROM   stock_bodega
        WHERE  id_bodega IN ($ph)
    ");
    $stmtStock->execute($misBodegasIds);
...
foreach ($stockRows as $r) {
    ...
    $stockMap[$bi][$pi] = array('stock' => (float)$r['stock_actual'], 'costo' => (float)$r['costo_promedio']);
}

$origenPre   = (int)get('id_bodega');
if (is_encargado() && !$origenPre && count($misBodegas) === 1) {
    $origenPre = (int)$misBodegas[0]['id'];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validación M:N
    if (is_encargado() && !in_array($id_bodega_origen, $misBodegasIds, true)) {
        $error = 'Solo puedes realizar traslados desde tus bodegas asignadas.';
    }
    ...
    if (!$error && ($id_bodega_origen <= 0 || $id_bodega_destino <= 0 || $fecha === '')) {
        $error = 'Bodega origen, destino y fecha son obligatorios.';
    } elseif (!$error && $id_bodega_origen === $id_bodega_destino) {
    ...
    if (!$error) {
        ...
        for ($i = 0; $i < $totalItems; $i++) {
            ...
            if ($id_producto > 0 && $cantidad > 0) {
                $detalleLimpio[] = array('id_producto' => $id_producto, 'cantidad' => $cantidad);
            }
        }
        ...
        if (!$detalleLimpio) {
            ...
            try {
                ...
                ");
                $stmt->execute(array(
                    $id_bodega_origen, $id_bodega_destino, $fecha,
                ...
                foreach ($detalleLimpio as $it) {
                    ...
                    if (!$stockOrigen) throw new Exception('Producto ID ' . $id_producto . ' no existe en la bodega origen.');
            ...
            <div class="row g-3 align-items-stretch">
                ...
                <div class="col-md-5">
                    ...
                    <?php if (is_encargado() && count($misBodegas) === 1): ?>
                        ...
                        <input type="hidden" name="id_bodega_origen" id="selOrigen" value="<?php echo (int)$b['id']; ?>">
                    <?php elseif (is_encargado()): ?>
                        <!-- Múltiples bodegas del encargado -->
                        <select name="id_bodega_origen" id="selOrigen" class="form-select form-select-lg" required>
                            <option value="">Seleccione una de tus bodegas...</option>
                            <?php foreach ($misBodegas as $b): ?>
                                <option value="<?php echo (int)$b['id']; ?>"
                                    <?php echo ($origenPre === (int)$b['id']) ? 'selected' : ''; ?>>
                                    <?php echo h($b['nombre'] . ' (' . $b['codigo'] . ')'); ?>
                                    <?php if ((int)$b['es_principal'] === 1): ?> ★<?php endif; ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                        <div class="form-text"><i class="bi bi-info-circle me-1"></i>Solo tus bodegas asignadas.</div>
                    <?php else: ?>
                        <!-- Admin: todas -->
                        <select name="id_bodega_origen" id="selOrigen" class="form-select form-select-lg" required>
    ...
    <div class="card shadow-sm border-0 mb-5" style="position:sticky; bottom:0; z-index:5;">
        <div class="card-body d-flex flex-wrap gap-3 align-items-center justify-content-between">
            <div class="d-flex gap-4 flex-wrap">
                <div>
                    <div class="text-muted small text-uppercase fw-bold">Productos</div>
                    <div class="h5 mb-0 fw-bold" id="resumenItems">0</div>
                </div>
                <div>
                    <div class="text-muted small text-uppercase fw-bold">Cantidad total</div>
                    <div class="h5 mb-0 fw-bold text-primary" id="resumenCantidad">0,00</div>
                </div>
                <div>
                    <div class="text-muted small text-uppercase fw-bold">Valor</div>
...
(function () {
    ...
    function renderProductos() {
        ...
        for (var i = 0; i < productos.length; i++) {
            ...
            if (st && parseFloat(st.stock) > 0) {
                disponibles.push({
                    ...
                    stock: parseFloat(st.stock), costo: parseFloat(st.costo)
                });
            }
        }
        ...
        for (var j = 0; j < disponibles.length; j++) {
            ...
            html += '<tr class="fila-producto" data-id="' + d.id + '" data-stock="' + d.stock + '" data-costo="' + d.costo
    ...
    function enlazarFilas() {
        ...
        for (var i = 0; i < filas.length; i++) {
            (function (tr) {
                ...
                btn.addEventListener('click', function () { inp.value = stock; validarFila(tr); recalcular(); });
            })(filas[i]);
        }
    }
    ...
    function recalcular() {
        ...
        var o  = getOrigenVal();
        var d  = parseInt(selDestino.value, 10) || 0;
        btnConfirmar.disabled = (items === 0) || !todoOk || !o || !d || (o === d);
    }
    ...
    buscador.addEventListener('input', function () {
        ...
        for (var i = 0; i < filas.length; i++) {
            var tr = filas[i];
            var match = (q === '' || tr.getAttribute('data-nombre').indexOf(q) >= 0 || tr.getAttribute('data-codigo').indexOf(q) >= 0);
    ...
    function validarBodegas() {
        if (getOrigenVal() && selDestino.value && String(getOrigenVal()) === selDestino.value) {
    ...
    if (selOrigen && selOrigen.tagName === 'SELECT') {
        selOrigen.addEventListener('change', function () { renderProductos(); validarBodegas(); });
    }
    ...
    document.getElementById('formTraslado').addEventListener('submit', function (e) {
        ...
        for (var j = 0; j < filas.length; j++) {
            ...
            if (chk.checked && validarFila(tr)) {
                var idp = document.createElement('input'); idp.type = 'hidden'; idp.name = 'item_id_producto[]'; idp.value = tr.getAttribute('data-id');
                var can = document.createElement('input'); can.type = 'hidden'; can.name = 'item_cantidad[]'; can.value = tr.querySelector('.inp-cantidad').value;
                this.appendChild(idp); this.appendChild(can);
                count++;
            }
        }
