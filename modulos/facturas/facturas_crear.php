...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // ... (El bloque de lógica de PHP se mantiene idéntico al original para asegurar el cálculo del stock)
        ...
        if ($existe) {
            ...
            if (!$detalleLimpio) {
                ...
                try {
                    ...
                    $stmt = $pdo->prepare("INSERT INTO facturas (id_bodega, id_proveedor, id_orden_compra, numero_factura, fecha_factura, fecha_recepcion, monto_neto, monto_iva, monto_total, estado, observacion, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                    $stmt->execute(array((int)$id_bodega, (int)$id_proveedor, ($id_orden_compra !== '' ? (int)$id_orden_compra : null), $numero_factura, $fecha_factura, ($fecha_recepcion !== '' ? $fecha_recepcion : null), $monto_neto, $monto_iva, $monto_total, $estado, $observacion, isset($_SESSION['user_id']) ? (int)$_SESSION['user_id'] : null));
                    ...
                    $stmtMov = $pdo->prepare("INSERT INTO movimientos_bodega (id_bodega, id_producto, tipo_movimiento, cantidad, precio_unitario, total, referencia_tipo, referencia_id, observacion, id_usuario) VALUES (?, ?, 'entrada_compra', ?, ?, ?, 'factura', ?, ?, ?)");
                    ...
                    foreach ($detalleLimpio as $item) {
                        ...
                        if (!empty($item['id_producto'])) {
                            $stmtMov->execute(array((int)$id_bodega, (int)$item['id_producto'], $item['cantidad'], $item['precio_unitario'], $item['subtotal'], $id_factura, 'Ingreso por factura N° ' . $numero_factura, isset($_SESSION['user_id']) ? (int)$_SESSION['user_id'] : null));
                            ...
                            if ($stockActual) {
                                ...
                                $pdo->prepare("UPDATE stock_bodega SET stock_actual = ?, costo_promedio = ? WHERE id = ?")->execute(array($nuevoStock, $item['precio_unitario'], (int)$stockActual['id']));
                            } else {
                                $pdo->prepare("INSERT INTO stock_bodega (id_bodega, id_producto, stock_actual, costo_promedio) VALUES (?, ?, ?, ?)")->execute(array((int)$id_bodega, (int)$item['id_producto'], $item['cantidad'], $item['precio_unitario']));
                            }
                        }
                    }

                    if ($id_orden_compra !== '') {
                        $pdo->prepare("UPDATE ordenes_compra SET estado = 'parcial' WHERE id = ? AND estado = 'pendiente'")->execute(array((int)$id_orden_compra));
                    }

                    $pdo->commit();
                    set_flash('success', 'Factura registrada y stock actualizado.');
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="h3 mb-0 text-gray-800"><i class="bi bi-receipt text-primary me-2"></i>Ingresar Nueva Factura</h1>
    <a href="facturas_lista.php" class="btn btn-outline-secondary"><i class="bi bi-arrow-left me-1"></i> Volver</a>
</div>
...
<form method="post" id="formFactura">
    <div class="card shadow-sm border-0 mb-4">
        ...
        <div class="card-body">
            <div class="row g-4">
                <div class="col-md-4">
                    <label class="form-label fw-bold text-secondary">Bodega de Destino <span class="text-danger">*</span></label>
                    <select name="id_bodega" class="form-select" required>
                        <option value="">Seleccione</option>
                        <?php foreach ($bodegas as $b): ?>
                            <option value="<?php echo (int)$b['id']; ?>" <?php echo (post('id_bodega') == $b['id']) ? 'selected' : ''; ?>><?php echo h($b['nombre']); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="col-md-4">
                    ...
                    <select name="id_proveedor" class="form-select" required>
                        ...
                        <?php foreach ($proveedores as $p): ?>
                            <option value="<?php echo (int)$p['id']; ?>" <?php echo (post('id_proveedor') == $p['id']) ? 'selected' : ''; ?>><?php echo h($p['razon_social'] . ' - ' . $p['rut']); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="col-md-4">
                    <label class="form-label fw-bold text-secondary">Orden de Compra (OC)</label>
                    <select name="id_orden_compra" class="form-select">
                        <option value="">Sin OC</option>
                        <?php foreach ($ocs as $oc): ?>
                            <option value="<?php echo (int)$oc['id']; ?>" <?php echo (post('id_orden_compra') == $oc['id']) ? 'selected' : ''; ?>><?php echo h($oc['numero_oc']); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="col-md-3">
                ...
                <div class="col-md-3">
                    ...
                    <select name="estado" class="form-select">
                        <option value="ingresada" selected>Ingresada (Activa)</option>
    ...
    <div class="card shadow-sm border-0 mb-4">
        <div class="card-header bg-white pt-3 pb-2 border-0 d-flex justify-content-between align-items-center">
            ...
            <button type="button" class="btn btn-sm btn-success" id="btnAgregarFila"><i class="bi bi-plus-lg me-1"></i> Agregar Producto</button>
        </div>
                            ...
                            <td>
                                <select name="item_id_producto[]" class="form-select item-producto">
                                    ...
                                    <?php foreach ($productos as $pr): ?>
                                        <option value="<?php echo (int)$pr['id']; ?>" data-nombre="<?php echo h($pr['nombre']); ?>"><?php echo h($pr['codigo'] . ' - ' . $pr['nombre']); ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </td>
        ...
        <div class="col-md-4 offset-md-8">
            <div class="card shadow-sm border-0 bg-light">
                <div class="card-body">
                    ...
                    <button type="submit" class="btn btn-primary w-100 mt-4 py-2"><i class="bi bi-floppy me-2"></i>Registrar e Ingresar a Stock</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
(function() {
    // Exactamente la misma lógica JS que usamos en ordenes de compra
